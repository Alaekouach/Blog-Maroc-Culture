...
try {
	 ...
	 if(isset($_SESSION['username'])){
      ...
      $requette2=$bdd->prepare('SELECT * FROM articles,utilisateurs,categorie WHERE articles.id_user=utilisateurs.id_user and articles.id_categorie=categorie.id_categorie and utilisateurs.id_user=?');
      $requette2->execute(array($_GET['id_user']));
	 ...
	 $requette3=$bdd->prepare('SELECT utilisateurs.username,articles.id_article,articles.titre,articles.img_article FROM articles,utilisateurs WHERE utilisateurs.id_user=articles.id_user ORDER BY articles.date_creation DESC LIMIT 4');
     $requette3->execute(array());
      
     //selection du username
     $requete_user= $bdd->prepare("SELECT username FROM utilisateurs WHERE id_user=? ");
     $requete_user->execute(array($_GET['id_user']));
...
<head>
    ...
    <title>Les articles d'un user</title>
...
<main>
    ...
    <div class="col-md-11 mt-5 ">
				<h5 class="container col-md-7 mt-3 pt-1 pb-1 text-info rounded text-center text-uppercase mob">Les articles Ecrits par : <span class="text-secondary"><?php echo $user['username'] ?></span></h5>
				<h3 class="container col-md-7 mt-3 pt-1 pb-1 text-info rounded text-center text-uppercase desk">Les articles Ecrits par : <span class="text-secondary"><?php echo $user['username'] ?></span></h3>
	</div>
    
	<div class="container col-md-11 d-flex">

        <div>   
			<div class=" container d-flex flex-wrap col-md-11 mt-3">
