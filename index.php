<?php
	 ...
	 $requette21=$bdd->prepare('SELECT * FROM articles,utilisateurs,categorie WHERE articles.id_user=utilisateurs.id_user AND articles.id_categorie=categorie.id_categorie ORDER BY articles.date_creation DESC LIMIT 6');
     $requette21->execute(array());
     
     $requette22=$bdd->prepare('SELECT * FROM articles,utilisateurs,categorie WHERE articles.id_user=utilisateurs.id_user AND articles.id_categorie=categorie.id_categorie ORDER BY articles.date_creation DESC LIMIT 6');
     $requette22->execute(array());
...
<main>
	...
	<div class="container col-md-11  mb-3 mt-4" >
	...
	<div class="col-md-11 mt-4 ">
				<!-- <h3 class="container col-md-7 mt-3 pt-1 pb-1 display-5 text-info rounded text-center text-uppercase">Les merveilles de l'artisanat marocain</h3> -->
    </div>

	<div class="container col-md-11 d-flex pt-1">
	<div class="mob">
			<div class="container col-md-11 bg-white pt-1 ">
				<h3 class="container col-md-7 mb-3 text-info rounded text-center text-uppercase titre">Blog sur la culture Marocaine</h3>
				<h6 class="container col-md-7 mt-2  text-center text-dark"> Al Maghrib, signifiant le “couchant” ou l’Occident. </h6>
				<p class="container col-md-10 mt-4 mb-5 pb-5 text-justify ">Porte de l’Afrique pour les Européens. Dépaysement assuré à quelques heures de Paris. Fès et sa magnifique médina. Marrakech et sa célèbre place Jamaa el Fna. Le sud marocain avec ses nombreuses kasbah et ses portes vers le désert du Sahara. Et tout le reste. Voyager au Maroc c’est aussi découvrir une cuisine variée: tajines, couscous, pâtisseries marocaines, thé à la menthe, …</p>
			</div>

			<div class=" container d-flex flex-wrap col-md-11 ">
				<?php  while($affiche_article=$requette21->fetch() ) {   ?>
			...
			<div class="mt-3 text-center"><u><a href="acceuil.php">Voir plus d'articles</a></u></div>
		...
		<div class="desk">
			...
			<div class=" container d-flex flex-wrap col-md-11 ">
				<?php  while($affiche_article=$requette22->fetch() ) {   ?>
					<div class="container col-md-5 mb-4 pt-2 bg-white">
						<button class="bouton5"><?php echo $affiche_article['intitule_categorie'] ?></button>
						<div class="">
							<a href="affiche_article.php?id_article=<?php echo $affiche_article['id_article'] ?>"><img src="<?php echo $affiche_article['img_article'] ?>" alt="" class="mb-2" style="width:100%"></a>
			...
			<div class="col-md-12 ">
				<?php  while($affiche_article_recent=$requette3->fetch() ) {   ?>
					...
					<div>
						...
						<small class="text-info text-uppercase float-right mb-3"><span class="text-dark">écrit par : </span> <?php echo $affiche_article_recent['username'] ?></small>
					</div>
				<?php   }  ?>
			</div>
